...
public:
	ConfigJSON();
	ConfigJSON(const char* data);
	ConfigJSON(JSON_Object* section);
	~ConfigJSON();
	...
	int Size() const;
	ConfigJSON GetSection(const char* section_name) const;
	ConfigJSON AddSection(const char* section_name);
	...
	ConfigJSON GetArray(const char* field, int index) const;
	...
	bool AddArrayEntry(const ConfigJSON& config);
