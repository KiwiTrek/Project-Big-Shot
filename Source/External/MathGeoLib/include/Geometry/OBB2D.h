...
/// A 2D arbitrarily oriented bounding box.
/** This data structure represents a box in 2D space. The local axes of this box can be arbitrarily oriented/rotated
...
public:
	...
	/** In the local space of the OBB, the center of this OBB is at (r.x,r.y), and the OBB is an AABB with
		size 2*r. */
	float2 pos;

	/// Stores half-sizes to x and y directions in the local space of this OBB2D. [similarOverload: pos]
	/** These members should be positive to represent a non-degenerate OBB2D. */
	float2 r;

	/// Specifies normalized direction vectors for the local axes. [noscript] [similarOverload: pos]
	/** axis specifies the +X direction in the local space of this OBB2D.
		The scale of this vector is always normalized. The half-length of the OBB2D along its local axes is
	...
	/** This means that the values of the members pos, r and axis are undefined after creating a new OBB2D using this
		default constructor. Remember to assign to them before use.
		@see pos, r, axis. */
	OBB2D() {}

	/// Constructs an OBB2D by explicitly specifying all its member values.
	/** @see pos, r, axis. */
	OBB2D(const float2 &pos, const float2 &r, const float2 &axis);

	/// Constructs an OBB2D from an AABB2D.
	/** Since the OBB2D is an AABB2D with arbirary rotations allowed, this conversion is exact, i.e. it does not loosen
		the set of points represented by the AABB2D. Therefore this constructor is implicit, meaning that you
		can directly assign an AABB2D to an OBB2D.
		@note Converting an OBB2D back to an AABB2D is not exact. See the MinimalEnclosingAABB2D() function for
			converting to the opposite direction.
		@see class AABB2D, SetFrom(), MinimalEnclosingAABB2D(). */
	OBB2D(const AABB2D &aabb);

	FORCE_INLINE static int NumFaces() { return 1; }
	FORCE_INLINE static int NumEdges() { return 4; }
	FORCE_INLINE static int NumVertices() { return 4; }

	/// Sets this structure to a degenerate OBB2D that does not have any volume.
	/** This function sets pos=(0,0), r = (-inf,-inf) and axis=float2(1,0) for this OBB2D.
		@note This function operates in-place. After calling this function, this OBB2D is degenerate.
	...
	/** This conversion is exact, and does not loosen the volume.
		@note Converting an OBB2D back to an AABB2D is not exact. See the MinimalEnclosingAABB2D() function for
			converting to the opposite direction.
		@param aabb The axis-aligned bounding box to convert to an OBB2D.
		@see classes AABB2D, MinimalEnclosingAABB2D(). */
	void SetFrom(const AABB2D &aabb);
	/** @param transform If a transformation matrix is supplied, this transformation is applied to the AABB2D before
...
//	void SetFrom(const AABB2D &aabb, const float2x2 &transform);
	...
	/** This function computes the tightest possible OBB2D (in terms of area) that contains the given circle, and stores the result in this structure.
		@note Since an OBB2D is a box, and Circle is, well, a circle, this conversion is not exact, but loosens the set of points in the representation.
		@see class Circle, MinimalEnclosingCircle(). */
